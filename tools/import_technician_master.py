...
def validate_and_transform(records: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for record in records:
        ...
        tech["ssn_last4"] = ssn_last4(clean(record.get("SSN / Tax ID")))
...
def fetch_actor(connection: sqlite3.Connection, username: str) -> sqlite3.Row:
    ...
    if actor is None:
        raise ValueError(f"Administrator user not found: {username!r}")
    if actor["role"] != "admin" or not actor["is_active"]:
        raise ValueError("The import actor must be an active administrator")
    return actor
...
def import_records(
    ...
    try:
        actor = fetch_actor(connection, actor_username)
