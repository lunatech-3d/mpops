...
class MarketService:
    ...
    def list_markets(self, include_inactive: bool = False) -> list[dict[str, Any]]:
        ...
        with self.auth.connection() as connection:
            rows = connection.execute(
                ...
                " ORDER BY market_name COLLATE NOCASE, market_id"
            )
    ...
    def search_markets(self, query: str, include_inactive: bool = False) -> list[dict[str, Any]]:
        ...
        where = " WHERE market_name LIKE ? COLLATE NOCASE"
    ...
    def create_market(self, session: Session, market_name: str) -> int:
        self._require_admin(session)
        name = self._clean_name(market_name)
        try:
            with self.auth.connection() as connection:
                cursor = connection.execute(
                    "INSERT INTO Markets (market_name,status,created_at,created_by) "
                    "VALUES (?,'Active',?,?)",
                    (name, utc_now_iso(), session.user_id),
                )
                ...
                record_event(
                    ...
                    details={"market_id": market_id, "market_name": name},
                )
                return market_id
        except sqlite3.IntegrityError as exc:
            raise ValueError("A market with that name already exists") from exc

    def update_market(self, session: Session, market_id: int, market_name: str) -> dict[str, Any]:
        self._require_admin(session)
        self._positive_id(market_id, "market_id")
        name = self._clean_name(market_name)
        try:
            with self.auth.connection() as connection:
                ...
                connection.execute(
                    "UPDATE Markets SET market_name=?,updated_at=?,updated_by=? "
                    "WHERE market_id=?",
                    (name, utc_now_iso(), session.user_id, market_id),
                )
                record_event(
                    connection,
                    "market_updated",
                    ...
                    details={
                        "market_id": market_id,
                        "before": {"market_name": before["market_name"]},
                        "after": {"market_name": name},
                    },
                )
