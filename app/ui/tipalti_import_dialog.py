"""Modal paste-and-preview wizard for Tipalti payment details."""
...
class TipaltiImportDialog(tk.Toplevel):
    def __init__(self, parent, service, session, batch_id: int, batch: dict, totals: dict,
        ...
        self.result = None
        self.title("Import Tipalti Payment Details")
        ...
        ttk.Label(outer, text="Import Tipalti Payment Details", style="Header.TLabel").pack(anchor="w")
        ttk.Label(outer, text="Copy the payment-detail rows from Tipalti and paste them below.\nInclude the header row when available.").pack(anchor="w", pady=(4, 8))
        ...
        self.summary_var = tk.StringVar(); self.balance_var = tk.StringVar()
    ...
    def paste(self):
        ...
        except tk.TclError: messagebox.showwarning("Tipalti Import", "The clipboard does not contain plain text.", parent=self); return
        self.text.insert("insert", value)

    def parse(self):
        try:
            result = parse_tipalti_text(self.text.get("1.0", "end-1c"))
            ...
            if not isinstance(exc, ValueError): LOGGER.exception("Unexpected Tipalti parser failure")
            messagebox.showerror("Tipalti Import", str(exc) or "The pasted data could not be parsed.", parent=self); return
        self.tree.delete(*self.tree.get_children())
        for row in self.result["rows"]:
            self.tree.insert("", "end", values=(row["source_row_number"], row["document_number"], row["document_type"] or "",
                row["document_date"] or "", row["description_raw"] or "", format_cents(row["amount_received_cents"]), row["status"], row["message"] or ""))
        ...
        proposal = import_preview_summary(self.batch["payment_amount_cents"], self.totals["imported_total_cents"], summary["importable_total_cents"])
        ...
        self.editor.pack_forget(); self.preview.pack(fill="both", expand=True, pady=8); self.summary_label.pack(anchor="w"); self.balance_label.pack(anchor="w", pady=(3, 0)); self.final.pack(fill="x", pady=(8, 0))
        self.import_button.configure(state="normal" if summary["valid_count"] else "disabled")

    def back(self):
        self.preview.pack_forget(); self.summary_label.pack_forget(); self.balance_label.pack_forget(); self.final.pack_forget(); self.editor.pack(fill="both", expand=True); self.text.focus_set()
    ...
    def import_rows(self):
        ...
        if not messagebox.askyesno("Confirm Tipalti Import", f"Import {summary['valid_count']} valid rows totaling {format_cents(summary['importable_total_cents'])} into this payment batch?\n\n{skipped}", parent=self): return
        ...
        try: imported = self.service.import_payment_items(self.session, self.batch_id, items)
        except Exception as exc:
            if not isinstance(exc, (ValueError, LookupError)): LOGGER.exception("Unexpected Tipalti import failure")
            messagebox.showerror("Tipalti Import", str(exc) or "The import failed.", parent=self); return
        messagebox.showinfo("Tipalti Import", f"Imported {imported['imported_count']} rows totaling {format_cents(imported['imported_total_cents'])}.", parent=self)
